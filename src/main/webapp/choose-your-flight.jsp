    ...
    for (Flights flight : flights) { %>
        ...
        <tr>
            <td> <a href="<%= buildQueryString(flight)%>">
                <%= flight.getParentAirline().getName()%> </a></td>
        <td> <%= flight.getPlaces().getSource()%></td>
        <td> <%= flight.getPlaces().getDestination()%></td>
        <td> <%= flight.getPrice()%></td>
        <td> <%= flight.getDepartureTime()%></td>  </tr>
   <% } %>

    <%! public String buildQueryString(Flights flights) throws UnsupportedEncodingException {
        return String.format("BookTicket.jsp?name=%s&from=%s&to=%s&price=%s&time=%s",
